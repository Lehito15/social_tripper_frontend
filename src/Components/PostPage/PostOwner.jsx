...
function PostOwner({ owner, date, bottomText, addUserToEvent, removeRequest, removeUser }) {
  const [showOptions, setShowOptions] = useState(false); // Stan zarządzający widocznością listy
  ...
  const handleRemoveUser = (userUuid) => {
    ...
    setShowOptions(false); // Zamknij listę po kliknięciu
    removeUser(userUuid)
  };
      ...
      {addUserToEvent && (
        <div className="post-owner-icons">
          <div className="accept-request" onClick={() => addUserToEvent(owner.uuid)}>
            <img src={`${process.env.PUBLIC_URL}/accept-green.png`} alt="Accept" className="post-owner-icon" />
          </div>
          <img src={`${process.env.PUBLIC_URL}/close.png`} alt="Close" className="post-owner-icon" onClick={() => addUserToEvent(owner.uuid)} />
        </div>
      )}
      ...
      {removeUser && (
        <div className="post-owner-icons">
          <img
            ...
            alt="More options"
            className="icon"
            onClick={toggleOptions} // Obsługa kliknięcia, która pokazuje/ukrywa listę
          />
          {showOptions && (
            <div className="leave-event" ref={dropdownRef}>
              <p onClick={() => handleRemoveUser(owner.uuid)} className="leave-event-text">Remove user</p>
            </div>
          )}
        </div>
      )}
