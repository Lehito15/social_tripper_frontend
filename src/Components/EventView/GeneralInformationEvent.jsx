...
function GeneralDetailsEvent({ description, publicText, publicIcon, eventCreated, maxMembers, isOwner, isGroup, locationScope, eventUUID, updateData, isPublic }) {
  ...
  const saveToDatabase = async () => {
    const updatedEvent = {
      ...
      isPublic: updatedPublicText === 'Public', // Zakładam, że 'Public' i 'Private' odpowiadają wartościom boolean.
      ...
      <div className='stats trip-info'>
        ...
        <div className="event-public ssp">
          ...
          {isEditing ? (
            ...
            options={options}
            value={options.find(option => option.value === (updatedPublicText.toLowerCase()))} // Dostosowanie do `updatedPublicText`
            isSearchable={false}
            onChange={(selectedOption) => {
              const selectedValue = selectedOption.value;
              setUpdatedPublicText(selectedValue === 'public' ? 'Public' : 'Private'); // Ustawienie stanu na nową wartość
            }}
          />
        ...
        <div className="event-public ssp">
          ...
          <p className="event-public-text ssp">
            {isGroup ? 'Group created' : 'Trip created'} at {eventCreated ? new Date(eventCreated).toLocaleDateString() : 'N/A'}
          </p>
        </div>
