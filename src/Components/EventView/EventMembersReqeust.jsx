...
`;
  ...
  const addUserToEvent = async (userUuid) => {
    try {
      ...
      await sendToBackend(endpoint, 'POST', JSON.stringify(userRequestEventDTO));
      setReload(!reload);
  ...
  return(
    <div>
      <Members title ={'Memebers Request'} members={data.requests} request={addUserToEvent} reload={setReload} />
