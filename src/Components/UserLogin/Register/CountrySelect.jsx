...
// Przygotowanie opcji do Select
const countries = Object.entries(phoneCodesToCountries).map(([code, country]) => ({
  value: country,
  label: (
    <div className="custom-option">
      <span className={'fi fi-' + countryToCode[country]} style={{ marginRight: '8px' }}></span>
      {country}
    </div>
  ),
}));
...
function CountrySelect({ data, updateData }) {
  ...
  return (
    <Select
      ...
      components={{
        IndicatorSeparator: () => null, // Usunięcie separatora
      }}
    />
  );
}
