...
function Chat({ friend, last_activity_date, messages}) {
  ...
  if(messages){
    ...
    if(!isRead){
      unreadCount = messages.filter(message => message.date > last_activity_date).length;
    ...
    const getTimeAgo = (date) => {
      ...
      if (diffInMinutes < 60) {
        ...
        return messageTime.format('DD/MM/YYYY'); // Zwracamy datę w formacie dzien/miesiąc/rok
      }
    };
  
    // Ustalamy tekst, który wyświetlimy
